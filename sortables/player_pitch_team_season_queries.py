...
table_name = 'player_pitch_team_season'
...
query_bio = """
    SELECT
        ...
        , bpid
        , full_name as name
        , date_part('day', concat(2020, '-07-01')::timestamp - birth_date::timestamp)/365.25 as age
        , bats
        , throws
...
query_standard = """
  SELECT
        ...
        , case when away_home = 'home' then gs.home_team else gs.away_team end as team_id
        , team_code
  ...
  join mlbapi.games_players gp on b.game_pk = gp.game_pk and b.pitcher_id = gp.player_id and player_type in ('pitcher','bullpen')
  join mlbapi.teams t on case when away_home = 'home' then gs.home_team else gs.away_team end = t.id
  where game_type = 'R' and left(status_code, 1) = 'F' 
  and season = {season}
  and level_id = {level_id}
  group by season, level_id, pitcher_id
      , case when away_home = 'home' then gs.home_team else gs.away_team end
      , team_code
...
query_ip = """
    SELECT 
        ...
        , case when bat_home_id = 1 then away_team else home_team end as team_id
    ...
    from (
        select
            season, level_id, game_pk, bat_home_id, pitcher_id, away_team, home_team
    ...
    group by season, level_id, pitcher_id, case when bat_home_id = 1 then away_team else home_team end
    ;
"""
...
query_bb = """
    SELECT
        season, level_id
        , pitcher_id as mlbid
        , case when half_inning = 'top' then home_team else away_team end as team_id
        --, count(distinct game_pk) as g
        --, count(*) as pa
    ...
    group by season, level_id, pitcher_id, case when half_inning = 'top' then home_team else away_team end
    ;
"""
...
query_field = """
    ...
    group by season, level_id, id, team;
"""
...
query_sraa = """
    SELECT distinct on (season, l.id, d.bpid)
        ...
        , sraa 
        , sraa_sd
    from models.sraa_pitcher_team_daily d 
    join xrefs.people_refs pr on d.bpid::int = pr.bpid and xref_type='mlb'
    join mlbapi.levels l on d.lvl = l.code
    join xrefs.teams_refs tr on d.teams_id = tr.teams_id and tr.xref_type='mlbam'
...
query_traa = """
    SELECT distinct on (season, l.id, d.bpid)
        ...
        , traa 
        , traa_sd
    from models.traa_pitcher_team_daily d 
    join xrefs.people_refs pr on d.bpid::int = pr.bpid and xref_type='mlb'
    join mlbapi.levels l on d.lvl = l.code
    join xrefs.teams_refs tr on d.teams_id = tr.teams_id and tr.xref_type='mlbam'
    where season = {season} and l.id = {level_id}
...
queries = [query_standard
            ...
            , query_sraa
            , query_opp
            #, query_warp  --need team version from JJ
        ]