drop materialized view if exists legacy_models.ip_split_team;
create materialized view if not exists legacy_models.ip_split_team as
select 
	season, level_id, league, t.id
	, avg(ip_g) as ip_g
...
and gs.level_id = 1 
group by season, level_id, league, t.id
;

grant select on legacy_models.ip_split_team to PUBLIC;
grant all privileges on legacy_models.ip_split_team to models;
