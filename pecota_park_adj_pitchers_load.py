...
vintage_last = session_dugout.query(func.max(Bp_pecota_pitching_park_adj.created_datetime)).scalar()  or datetime.strptime('01/01/01 01:01:01', '%m/%d/%y %H:%M:%S')
vintage_last_judge = session_cage.query(func.max(Pecota_padj_pitchers.vintage)).scalar() 
print(vintage_last_judge ,vintage_last)
if vintage_last_judge >   vintage_last:
    pecota_raw = session_cage.query(Pecota_padj_pitchers).filter(Pecota_padj_pitchers.vintage == vintage_last_judge ).all()
    session_dugout.query(Bp_pecota_pitching_park_adj).delete()   
